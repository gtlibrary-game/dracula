...
{
    public static MessageSigner CreateMessageSigner(GameObject parent, string messageToSign, int accountIndex = 0, bool disconnectOnSign = false, string addressToSign = null)
    {
    ...
    {
        if (!WalletConnect.Instance.Session.Connected)
        {
            throw new IOException("[WalletConnect] No ActiveSession is connected to sign the message");
        }

        var address = WalletConnect.Instance.Session.Accounts[accountIndex];
        ...
        var result = await WalletConnect.Instance.Session.EthPersonalSign(address, messageToSign);
        ...
        if (disconnectOnSign)
        {
            await WalletConnect.Instance.Session.Disconnect();
        }
