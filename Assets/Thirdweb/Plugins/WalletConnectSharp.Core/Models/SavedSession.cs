    ...
    public class SavedSession
    {
        ...
        private sealed class SavedSessionEqualityComparer : IEqualityComparer<SavedSession>
        {
            public bool Equals(SavedSession x, SavedSession y)
            {
                ...
                return x.ClientID == y.ClientID
                    && x.BridgeURL == y.BridgeURL
                    && x.Key == y.Key
                    && Equals(x.KeyRaw, y.KeyRaw)
                    && x.PeerID == y.PeerID
                    && x.NetworkID == y.NetworkID
                    && Equals(x.Accounts, y.Accounts)
                    && x.ChainID == y.ChainID
                    && Equals(x.DappMeta, y.DappMeta)
        ...
        protected bool Equals(SavedSession other)
        {
            return ClientID == other.ClientID
                ...
                && Key == other.Key
                && Equals(KeyRaw, other.KeyRaw)
                && PeerID == other.PeerID
                && NetworkID == other.NetworkID
                && Equals(Accounts, other.Accounts)
