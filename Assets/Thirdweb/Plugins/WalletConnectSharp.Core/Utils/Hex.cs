...
namespace WalletConnectSharp.Core.Utils
{
    public static class HexByteConvertorExtensions
    {
        ...
        public static byte[] HexToByteArray(this string value)
        {
            ...
            {
                throw new FormatException(string.Format("String '{0}' could not be converted to byte array (not hex?).", value), ex);
            }
        }
        ...
        private static byte FromCharacterToByte(char character, int index, int shift = 0)
        {
            ...
            {
                throw new FormatException(string.Format("Character '{0}' at index '{1}' is not valid alphanumeric character.", character, index));
            }
